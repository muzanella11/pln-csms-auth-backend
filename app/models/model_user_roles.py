...
class ModelUserRoles(Models):
    ...
    def get_list(self):
        sql_rows = self.execute("SELECT id, name, label, privilege_type, {}, {} from `{}`".format(self.convert_time_zone('created_at'), self.convert_time_zone('updated_at'), self.table_name))
    ...
    def get_detail_by(self, columns = None, value = None):
        ...
        sql_rows = self.execute("SELECT id, name, label, privilege_type, {}, {} from `{}` WHERE `{}` = '{}'".format(self.convert_time_zone('created_at'), self.convert_time_zone('updated_at'), self.table_name, columns, value))
    ...
    def create_data(self, value = None):
        ...
        action['{}'.format(self.table_name)] = {
            ...
            'command': (
                ...
                " ('{}','{}','{}', NOW())".format(value.get('name'), value.get('label'), value.get('privilege_type'))
            )
        }

...
class ModelUserSession(Models):
    ...
    def get_list(self):
        sql_rows = self.execute("SELECT id, token, expired, {}, {} from `{}`".format(self.convert_time_zone('created_at'), self.convert_time_zone('updated_at'), self.table_name))
    ...
    def get_detail_by(self, columns = None, value = None):
        ...
        sql_rows = self.execute("SELECT id, token, expired, {}, {} from `{}` WHERE `{}` = '{}'".format(self.convert_time_zone('created_at'), self.convert_time_zone('updated_at'), self.table_name, columns, value))
    ...
    def create_data(self, value = None):
        ...
        action['{}'.format(self.table_name)] = {
            ...
            'command': (
                ...
                " ('{}','{}', NOW())".format(value.get('token'), value.get('expired'))
            )
        }
